...
\textbox{光是粒子还是波：一个历史上的争论}{
    \titem{
        ...
        \item 1801年，托马斯·杨\footnote{Thomas Young（1773－1829），英国科学家、医生、通才。——译者注。}做了一个双缝实验。在那之后的成果发展（例如泊松点）反映波动理论再次占据主导地位。
        \item In 1861, Maxwell published his equations of E\&M. Electric field, magenetic field and light is then unified in one theory, where light appeared in the form of wave solutions. 1861 年，麦克斯韦\footnote{James Clerk Maxwell（1831－1879），英国苏格兰数学物理学家。——译者注。}发表了他的E\&M方程。这个方程把电场、磁场和光统一在一个理论中，其中光以波的形式出现。
    }
}

Maxwell's equations concludes the  debate about the \emph{classical} nature of light -- light is wave, and its mathematical equations are found from the first principle. However, the nature of light starts to look surprisingly different again once we step into the modern era.麦克斯韦方程组结束了关于光的\emph{经典}性质的争论：光是波。它的数学方程是从第一原理找到的。然而，如果我们在现代观察，光的本质又开始变得惊人地不同。

The whole thing started at an ``ultraviolet catastrophe''\index{ultraviolet catastrophe}\index{black body radiation}. The observed radiation from black body does not agree with Boltzmann's theory of statistical physics for short wavelength light. In 1900, Planck suggested that light is emitted and absorbed in a quantized way to solve the problem. This is the first indication of the quantum world historically. Here we will not follow a historical order, but instead show you two more intuitive experiments made slightly later, where classical physics fails.整个事件开始于``紫外灾变（瑞利-金斯灾变）''\index{紫外灾变}\index{黑体辐射}。观察到的黑体辐射与玻尔兹曼关于短波长光的统计物理理论不符。 1900年，普朗克提出以量子化的方式发射和吸收光来解决这个问题。这是历史上量子世界的第一个迹象。在这里，我们不会遵循历史顺序，而是向您展示两个稍后进行的更直观的实验，其中经典物理学失败了。
